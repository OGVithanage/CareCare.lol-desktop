...
- The report's shared-IP limitation applies: allowing an IP permits every hostname and port on that IP. Domains do not include subdomains automatically. Redirects, CDNs and page assets may need additional entries. Loopback proxies can forward traffic to otherwise allowed endpoints; this first build is not exact hostname enforcement or a defense against every tunneling technique.
...
2. Add a controlled domain with A and AAAA records. Verify both families can connect immediately; verify other destination IPs remain blocked. Add any explicit redirect/asset domains needed by browsers.
